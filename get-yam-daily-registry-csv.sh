...
IMPORT_PAYMENT_FLAG='NO'
IMPORT_REFUND_FLAG='NO'
# IMPORT='NO'
IMPORT_PAYMENT='NO'
IMPORT_ITEM='NO'
IFS_BCK=$IFS
...
#yoomoney_payments
#yoomoney_refunds_137248_2024_10_02.csv
if ls *yoomoney_*.csv > /dev/null 2>&1; then
    # echo "По крайней мере один файл найден."

    rm -f *yamregister*

    CSV_LIST=$(ls -1 -- yoomoney_*.csv)
    for csv in $CSV_LIST
    do
     mv $csv yamregister_$csv.org
    done
fi
...
do
  ...
  ROWS=$(grep -c ';RUB;' "$txt")  # BOTH payments OR refunds
  #ROWS=$(awk -F '[ :]' '$0 ~ /Число платежей:/ {print $4}' "$txt")
  # REFUNDS=$(grep -c 'REF' "$txt")
  #REFUNDS=$(grep -c ';RUB;' "$txt")

  logmsg INFO "Checking name of $txt"
  if [[ "$txt" == *refunds* ]]; then
    REF_FLAG=TRUE
    logmsg INFO "== refunds REF_FLAG=$REF_FLAG"
  elif [[ "$txt" == *payments* ]]; then
    PMNT_FLAG=TRUE
    logmsg INFO "== payments PMNT_FLAG=$PMNT_FLAG"
  fi
  logmsg INFO "REF_FLAG=$REF_FLAG"
  logmsg INFO "PMNT_FLAG=$PMNT_FLAG"
  ...
  then 
     ...
     logmsg INFO "reg_date={$reg_date}"
IFS=- read loc_year loc_mon loc_day <<EODATE
${reg_date}
EODATE
     ...
     then
        ...
        logmsg INFO "PAYMENTS TXT file $txt contains $ROWS rows. Prepare \\COPY command to load CSV into PG"
        awk '/;RUB;/ {N=split($0,arr,";"); for (i=1;i<=14;i++) printf "%s;", arr[i]; print ";"}' "$txt" |sed -e 's/; /;/g' -e "s/$/$reg_date;/" > "$PG_CSV"
        ...
        IMPORT_PAYMENT_FLAG='YES'
        logmsg INFO 'set IMPORT_PAYMENT_FLAG=YES'
        PMNT_FLAG=FALSE
     fi
     # 
     if [ +"$REF_FLAG" = +TRUE ]
     then
        ...
        logmsg INFO "REFUNDS TXT file $txt contains rows. Prepare \\COPY command to load CSV into PG"
        awk '/;RUB;/ {N=split($0,arr,";"); for (i=1;i<=10;i++) printf "%s;", arr[i]; print ";"}' "$txt" |sed -e 's/; /;/g' -e "s/;$/$reg_date/" > "$PG_CSV_RET"
        ...
        IMPORT_REFUND_FLAG='YES'
        logmsg INFO 'set IMPORT_REFUND_FLAG=YES'
        REF_FLAG=FALSE
     fi
...
# use ~/.pgpass
logmsg INFO "final REF_FLAG=$REF_FLAG"
logmsg INFO "final PMNT_FLAG=$PMNT_FLAG"
logmsg INFO "================== Start import to PG"
if [ $IMPORT_PAYMENT_FLAG == 'YES' ]
then
   ...
   if [ $RC_IMP -eq 0 ]
   then 
      logmsg INFO "CSV {$PG_CSV} successfully loaded into $PG_SRV"
      ### NO MORE order# in CSV!!!
      #ORDERS_SET=`awk -F ";" '$2 ~ / #[0-9]+/ {split($2, arr, "#"); s=s arr[2]","}END{gsub(/\"/, "", s); printf "%s", substr(s, 1, length(s)-1) }' $PG_CSV` 
      # NEW
      # get 1st field = yam_id
      ...
      # find yam_ids in yampayment

      SEL_YIDS="SELECT string_agg(order_id::varchar, ', ') FROM yampayment WHERE yam_id IN ($OL);"
      #echo $SEL_YIDS                                                                   
      # get ORDERS_LIST
      ORDER_SET=$(psql -h $PG_SRV -U arc_energo -d arc_energo -A -t -c "$SEL_YIDS") 

      logmsg INFO "ORDER_SET={$ORDER_SET}"
      if [ +"$ORDERS_SET" != '+' ]
      then
...
# use ~/.pgpass
# PROD if [ $IMPORT_PAYMENT != 'NO' ] # csv filename
if false
then
   logmsg INFO "Import payments from $IMPORT_PAYMENT into $PG_SRV"
   cat $IMPORT_PAYMENT
...
# use ~/.pgpass
# PROD if [ $IMPORT_ITEM != 'NO' ] # csv filename
if false
then
   logmsg INFO "Import items from $IMPORT_ITEM into $PG_SRV"
   cat $IMPORT_ITEM
